...
class LigneItem extends Component {
  ...
  componentDidMount = () => {
    ...
    let $curValProduire = $(a_produire).val();
    $curValProduire = $curValProduire.split(' ');
    $curValProduire = $curValProduire.join('');
          ...
          if(valeur){
            ...
            if(react.props.currentSelect.includes(valeur)){
              swal({
                  ...
                  text: 'Le produit se trouve deja les elements de la recette. Vous ne pouvez plus l\'utiliser.',
                  type: 'error',
      ...
      this.setState(newState, () => {
        if (this.props.action === 'add' && !$.isEmptyObject(data)){
          ...
          this.props.updateMatiere(this.props.index, data.name, 'name')
        	...
        	this.props.updateMatiere(this.props.index, data.cout, 'cout')
        }
      })
    });
  ...
  render = () => {
    ...
    return (
            <table className="table table-line table-condensed articles-lines" id={this.props.index}>
              <tbody>
                  <tr>
                    <td style={{ width:'50%' }} className="no-padding fieldSelect">
                    ...
                    <td className=" text-right" style={{ width: "30%" }}>
                      <input type="text" name="cout" defaultValue={this.state.data.cout} readOnly className="form-control-plaintext text-right" />
                    </td>
