...
prefix = Blueprint('api_vente', __name__)
...
def make_public(data):
    ...
    for field in data:
        if field == 'id':
            ...
            new_task['uri_view'] = url_for('vente.view', data_id=data['id'], _external=True)
        else:
...
def index(compte_id):
    ...
    if q is not None:
        datas = Documents.objects(Q(compte=compte_id) & Q(reference__icontains=q) & Q(type_transaction=4))
    else:
        datas = Documents.objects(Q(compte=compte_id) & Q(type_transaction=4))
    ...
    count = Documents.objects(Q(compte=compte_id) & Q(type_transaction=4)).count() / limit
...
def check():
    ...
    prix_vente = request.json['prix_vente']
    quantite = request.json['quantite']
    magasin_origine = request.args.get('magasin_origine')
    ...
    if article_id :
        ...
        if select:
            info['prix_vente'] = variante.prixVente(magasin_origine)
        else:
            info['prix_vente'] = int(prix_vente)

        info['magasin'] = variante.MagVarianteID()

        info['montant'] = float(info['prix_vente']) * float(info['quantite'])

    return jsonify(info)

@prefix.route('/all/item/<objectid:compte_id>/', methods=['GET'])
def allArticle(compte_id):
    ...
    if magasin:
        articles = Articles.objects(Q(compte=compte_id) & Q(type_article=0) & Q(a_vendre=1))
        for article in articles:
            if len(article.magasins) and magasin in article.magasins:
                for variante in article.variantes:
                    datas.append(variante)

            if len(article.magasins) == 0:
                for variante in article.variantes:
                    datas.append(variante)
...
def ligne(compte_id, item_id=None):
    ...
    if item_id :
        ...
        for item in docs.ligne_data:
            ...
            matiere['prix_vente'] = int(item.prix_vente)
            matiere['montant'] = int(item.quantite) * int(item.prix_vente)
